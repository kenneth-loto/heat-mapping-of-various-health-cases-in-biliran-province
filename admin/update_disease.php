...
$disease = $function->getADisease($id);

$categories = $function->getDiseaseCategories();  // Assuming you get categories from the database
?>
                ...
                <form method="post" action="../navigate.php" enctype="multipart/form-data">
                    <div class="row text-dark">
                        ...
                        <div class="col-md-12 mb-4">
                            ...
                            <input type="text" class="form-control" name="name_of_disease" value="<?= isset($_SESSION['form_data']['name_of_disease']) ? htmlspecialchars($_SESSION['form_data']['name_of_disease']) : htmlspecialchars($disease->name_of_disease); ?>" placeholder="e.g. Diabetes" required>
                        </div>

                        <!-- Short Description -->
                        <div class="col-md-12 mb-4">
                            <label class="form-label" for="short_description">Short Description <span class="text-danger">*</span></label>
                            <textarea class="form-control" name="short_description" placeholder="Diabetes is a chronic condition that affects how the body processes blood sugar, leading to high glucose levels." required> <?= isset($_SESSION['form_data']['short_description']) ? htmlspecialchars($_SESSION['form_data']['short_description']) : htmlspecialchars($disease->short_description); ?></textarea>
                        </div>
                        ...
                        <div class="col-md-12 mb-4" id="new_category_input" style="display: <?= ($category->disease_category == 'not_in_category') ? 'block' : 'none'; ?>;">
                            <label class="form-label" for="new_category">New Category <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="new_category" id="new_category" placeholder="Enter new category" value="<?= isset($_SESSION['form_data']['new_category']) ? htmlspecialchars($_SESSION['form_data']['new_category']) : ''; ?>">
                        </div>

                        <!-- Centered Buttons -->
                        <div class="d-flex justify-content-center">
                            <button type="submit" class="btn btn-primary me-3 w-25 py-2 fs-4 mb-4 rounded-2" name="btn-update-disease" onclick="return confirm('Are you sure you want to submit? Please review your details before proceeding.');" title="Submit" style="justify-content: center !important;">Submit</button>
